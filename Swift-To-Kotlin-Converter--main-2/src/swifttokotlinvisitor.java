...
public class swifttokotlinvisitor extends Swift3BaseVisitor<String> {
    ...
    public String visitTop_level(Swift3Parser.Top_levelContext ctx) {
        ...
        String kotlin_import="";
//        System.out.println(ctx.getText());
        if(kotlinCode.contains("NavHost")){
            kotlin_import+="import androidx.navigation.NavHostController\n";
        }
//        if(kotlinCode.contains("MainActivity : ComponentActivity()")){
//            kotlin_import+="import android.os.Bundle\n" +
//                    "import androidx.activity.ComponentActivity\n" +
//                    "import androidx.activity.compose.setContent\n";
//        }
        if(kotlinCode.contains("MainActivity : ComponentActivity()")){
            kotlin_import+="package com.example.tastemade\n\n" +
                    "import android.os.Bundle\n" +
                    "import androidx.activity.ComponentActivity\n" +
                    "import androidx.activity.compose.setContent\n";
        }
        return kotlin_import+"\n"+kotlinCode;
    }
...
//        System.out.println(ctx.getText());
        try {
            ...
            return "Error in visit statment";
        }
        return kotlin_statment;
    }
    ...
    public String visitDeclaration(Swift3Parser.DeclarationContext ctx) {
        ...
        try {
            ...
            if (ctx.variable_declaration() != null) {
                ...
                kotlin_declartion+=visitConstant_declaration(ctx.constant_declaration());
...
//        Struct_declration += "struct ";
        int index =ctx.getText().indexOf("Taste_made_2App");
        if(index !=-1){
            ...
            Struct_declration+="@Composable\n";
            Struct_declration+="fun ";
            Struct_declration += visitStruct_name(ctx.struct_name());
            Struct_declration+="(" ;
            if(ctx.getText().contains("RecipeDetailsView")){
                Struct_declration+="recipe: Recipe";

            }
            if(ctx.getText().contains("RecipeView")){
                Struct_declration+="recipes: List<Recipe>";

            }
            Struct_declration+=")";
            Struct_declration += "{\n" + visitStruct_body(ctx.struct_body()) + "\n}";
        }
        else if (ctx.getText().contains("Identifiable")){
            Struct_declration+="data class ";
            Struct_declration+= visitStruct_name(ctx.struct_name());;
            Struct_declration+="("+visitStruct_body(ctx.struct_body())+")";
...
//        }
    ...
    public String visitStruct_name(Swift3Parser.Struct_nameContext ctx) {
        ...
        if (ctx.getText().contains("Taste_made_2App")){
...
//            System.out.println(ctx.getText());
    ...
    public String visitConstant_declaration(Swift3Parser.Constant_declarationContext ctx) {
        String constant="";
        constant+="val ";
        if(ctx.getText().contains("imageName:String")){
            constant+="imageResId: Int";
        }
        else {
            constant += visitPattern_initializer_list(ctx.pattern_initializer_list());
            constant+=",";
        }
...
//        System.out.println(ctx.getText());
        if(ctx.getText().contains("var body: some")){
            System.out.println("sdsd");
            kotlinVariableDeclaration+="var body: some View {";
...
//        kotlinVariableDeclaration+=visitCode_block(ctx.code_block());
...
//        }
//        else {
            if(ctx.type_annotation().getText().contains("[Recipe]")){
                kotlinPattern+= ": ArrayList<Recipe>";
            }
            if(ctx.type_annotation().getText().contains("Recipe")){
                kotlinPattern+= ": List<Recipe>";
            }
            else {
...
//                    }
                        ...
                        * */
                        ...
                        if(ctx.getText().contains("VStack")||ctx.getText().contains("HStack") ||ctx.getText().contains("ZStack")||ctx.getText().contains("NavigationView")||ctx.getText().contains("Foreach")||ctx.getText().contains("NavigationLink")){
                        ...
                        if(Swift3Parser.Function_call_expressionContext.class.isInstance(ctx.postfix_expression())){
                        ...
                        if(ctx.getText().contains("Text(\"Breakfast\")")){
                            Java_prefix+="RecipeView(recipes = breakfastRecipes";}
                        if(ctx.getText().contains("RecipeView(recipes: breakfastRecipes)")) {
                            Java_prefix += "breakfast";
                        }
                        ...
                        else if (Swift3Parser.PrimaryContext.class.isInstance(fnctx.postfix_expression())) {
...
//                            Java_prefix += visitPrimary(pctx);
                        ...
                        Java_prefix += subctx.expression_list().getText() + ")";
                          ...
                          if(ctx.getText().contains("recipe.imageName")){
                            Java_prefix+="painterResource(recipe.imageResId), contentDescription = null";
                        }
//                          if(ctx.getText().contains("\\(recipe.ingredients)")){
//                              Java_prefix+="Ingredients: ${recipe.ingredients}";
//                          }
                         else {
...
//        System.out.println(ctx.getChild(0) + " "+ ctx.getChild(1));
    ...
    @Override public String visitDeclaration_identifier(Swift3Parser.Declaration_identifierContext ctx) {
        ...
        if (ctx.getText().contains("Text")) {
            kotlin_delcaration += "Text";
        }
...
//            kotlin_delcaration+=;
        ...
        } else if (ctx.getText().contains("RecipeView")){
...
//            literal+="[\n";
            ...
            literal+=visitArray_literal(ctx.array_literal());
//            literal+="\n]";
        }
        if(ctx.getText().contains("Ingredients: \\(recipe.ingredients")){
            literal+="Ingredients: ${recipe.ingredients}";
        }
        return literal;

    }
    @Override public String visitArray_literal(Swift3Parser.Array_literalContext ctx) {
        String array_literal="";
            array_literal += visitArray_literal_items(ctx.array_literal_items());

        return array_literal;
    }
    @Override public String visitArray_literal_items(Swift3Parser.Array_literal_itemsContext ctx) {
        String literal_items="";
        literal_items+=visitArray_literal_item(ctx.array_literal_item());


        return literal_items;
    }
    @Override public String  visitArray_literal_item(Swift3Parser.Array_literal_itemContext ctx) {
        String literal_item="";
        literal_item+=visitExpression(ctx.expression());
        return literal_item;
    }
...
//        System.out.println(ctx.getText());
        ...
        try {
            ...
            if (ctx.function_call_argument_list() != null) {
                Java_clause += "(";
                if(ctx.getParent().getText()=="Spacer"){
                    Java_clause+="Modifier.weight(1f)";
                }
...
//        kotlin_function_list+=ctx.getText();
    ...
    public String visitFunction_call_argument(Swift3Parser.Function_call_argumentContext ctx) {
        ...
        try {
            System.out.println(ctx.getText());
            if (ctx.getText().contains("imageName: \"omelette")){
                kotlin_function_call_argument+="imageResId";
//                kotlin_function_call_argument+=;

            }
            if(ctx.getText().contains("recipes:breakfastRecipes")){
                kotlin_function_call_argument+="\"breakfast\"";
            }
            if(ctx.getText().contains("recipes:lunchRecipes")){
                kotlin_function_call_argument+="\"lunch\"";
            }
            if(ctx.getText().contains("recipes:dinnerRecipes")){
                kotlin_function_call_argument+="\"dinner\"";
            }
...
//                }
    ...
    public String visitLabel_identifier(Swift3Parser.Label_identifierContext ctx) {
        ...
        try {
            ...
            } else if(ctx.getText().contains("imageName")){


                label_identifer+="imageResId";

            } else if(ctx.getText().contains("destination")){
    ...
    public String visitFunction_call_expression_with_closure(Swift3Parser.Function_call_expression_with_closureContext ctx){
        ...
        if(ctx.postfix_expression().getChild(0).getText().contains("NavigationView")){
            Kotlin_output+="val navController = rememberNavController()\n";

            Kotlin_output+="NavHost";
        }
...
//        }
        ...
        if(ctx.postfix_expression().getChild(0).getText().contains("VStack")){
            ...
            Kotlin_output+="Box";
        } else if(ctx.postfix_expression().getChild(0).getText().contains("NavigationLink")){
            Kotlin_output+="composable";
        }
